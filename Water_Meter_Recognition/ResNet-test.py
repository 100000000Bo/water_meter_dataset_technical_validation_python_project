...
class MeterDatasetTest(Dataset):
    ...
    Labels are extracted from the first five characters of the filename.
    """
...
class ResNetMeter(nn.Module):
    """
    ResNet18-based multi-digit recognition network.
    The network predicts five digits simultaneously.
    """
    def __init__(self):
        super(ResNetMeter, self).__init__()
...
model = ResNetMeter().to(device)
model.load_state_dict(
    torch.load('water_meter_resnet_model.pth', map_location=device)
)
model.eval()
