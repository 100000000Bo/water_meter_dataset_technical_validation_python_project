...
class DenseNet_Meter(nn.Module):
    """
    DenseNet-based model for five-digit water meter recognition.
    """
    def __init__(self):
...
model.load_state_dict(
    torch.load("model/water_meter_densenet_model.pth", map_location=device)
)
model.eval()
...
def evaluate_and_save_predictions(
    ...
    dataloader,
    allowed_error=1,
    output_file="predictions_densenet.csv"
):
    """
    Evaluate the model under exact-match and allowed-error criteria,
