...
class MeterDataset(Dataset):
    """
    Custom dataset for water meter digit recognition.
    The label is extracted from the first five characters of the image filename.
    """
...
class DenseNetMeter(nn.Module):
    """
    DenseNet-based multi-digit classification network.
    The model predicts five digits simultaneously, each with ten possible classes.
    """
    def __init__(self):
        super(DenseNetMeter, self).__init__()
    ...
    def forward(self, x):
        x = self.features(x)
        x = nn.AdaptiveAvgPool2d(self.pool_size)(x)
        x = torch.flatten(x, 1)
        x = self.classifier(x)
...
def evaluate_and_save_predictions(model, test_loader,
    ...
    """
    Evaluate the model on the test set and save predictions to a CSV file.
    """
    model.eval()
    correct_full = 0
    correct_allowed = 0
