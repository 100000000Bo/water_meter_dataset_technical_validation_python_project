...
# 定义模型结构
class ResNet_Meter(nn.Module):
    def __init__(self):
        super(ResNet_Meter, self).__init__()
        resnet = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)  # 使用预训练权重
    ...
    def forward(self, x):
        x = self.features(x)
        x = torch.flatten(x, 1)  # 展平，输出形状 (batch_size, resnet.fc.in_features)
        x = self.fc(x)
        return x.view(-1, 5, 10)  # 输出形状为 (batch_size, 5, 10)

# 加载模型
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = ResNet_Meter().to(device)
model.load_state_dict(torch.load('water_meter_resnet_model.pth', map_location=device))
model.eval()
