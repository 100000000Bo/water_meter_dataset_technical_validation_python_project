...
class CNN_Meter(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1),  # 输出 (32, 60, 200)
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  # 输出 (32, 30, 100)

            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # 输出 (64, 30, 100)
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  # 输出 (64, 15, 50)

            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # 输出 (128, 15, 50)
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)  # 输出 (128, 7, 25)
        )
    ...
    def forward(self, x):
        x = self.features(x)
        x = torch.flatten(x, 1)  # 展平，输出形状 (batch_size, 128 * 7 * 25)
        x = self.classifier(x)
...
model.load_state_dict(torch.load('water_meter_cnn_model.pth', map_location=device))
model.eval()
