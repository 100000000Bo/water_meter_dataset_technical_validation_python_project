...
def train_model(model, dataloader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        for images, masks in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            outputs = F.interpolate(outputs, size=(512, 512), mode="bilinear", align_corners=False)  # 上采样到目标尺寸
...
model.decode_head.classifier = nn.Conv2d(256, 1, kernel_size=(1, 1))  # 二分类，输出通道改为1
...
# 保存模型
torch.save(model.state_dict(), "segformer_water_meter.pth")
print("模型已保存为 segformer_water_meter.pth")
